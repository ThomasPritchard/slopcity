...
export class GuestRepository {
 ...
 async initialise() {
  await this.transaction(async client => {
   ...
   if (versions.rows.some(row => row.version !== 1 && row.version !== 2 && row.version !== 3 && row.version !== 4 && row.version !== 5 && row.version !== 6 && row.version !== 7 && row.version !== 8 && row.version !== 9 && row.version !== 10)) throw new Error('Unsupported guest schema version');
