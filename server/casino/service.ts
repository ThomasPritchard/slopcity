...
export class CasinoService {
 ...
 constructor(readonly roomId: string, readonly repository: CasinoRepository, private hooks: CasinoHooks, options: { random?: Random; now?: () => number; shoe?: () => Card[] } = {}) {
  ...
  const now = this.now();
  this.roulette = { view: { id: 'roulette-1', game: 'roulette', roundId: randomUUID(), phase: 'betting', deadline: now + ROULETTE_BETTING_MS, result: null, betCount: 0, history: [] }, bets: [], outcome: null };
  for (const id of ['blackjack-1', 'blackjack-2'] as const) this.blackjack.set(id, { view: { id, game: 'blackjack', roundId: randomUUID(), phase: 'betting', deadline: now + BLACKJACK_BETTING_MS, dealer: [], dealerTotal: null, seats: [], activeSeat: null, activeHand: null }, seats: new Map(), cards: [], dealer: [], revealed: false });
 ...
 private state(): CasinoState {
  ...
  const tables: CasinoTableView[] = [{ ...r.view, history: [...r.view.history], phase: this.blocked(r.view.id) ? 'paused' : r.view.phase, betCount: r.bets.length }];
 ...
 private publish() { this.hooks.publish(this.state()); }
 private privateState(profileId: string) { this.hooks.private(profileId, 'casino-private', { rouletteBets: this.roulette.bets.filter(b => b.profileId === profileId).map(b => ({ wagerId: b.wagerId, roundId: this.roulette.view.roundId, bet: b.bet })) }); }
 ...
 private parse(value: unknown): CasinoCommand {
  ...
  if (v.action === 'roulette-bet') { if (tableId !== 'roulette-1') fail('invalid_table', 'Choose the roulette table'); return { action: 'roulette-bet', requestId, tableId: 'roulette-1', roundId, bet: rouletteBet(v.bet) }; }
 ...
 private financial(profileId: string, command: CasinoCommand, fingerprint: string, sessionId: string): Pending {
  if (command.action === 'sync' || command.action === 'leave' || command.action === 'blackjack-join') return fail('invalid_action', 'Invalid financial action');
  let departed = false;
  let stake: number, roundId: string, details: Record<string, unknown>, validate: () => void, apply: (result: WagerResult) => void;
  if (command.action === 'roulette-bet') {
   const table = this.roulette; stake = command.bet.stake; roundId = command.roundId; details = { game: 'roulette', bet: command.bet };
   validate = () => {
 ...
 private unoccupied(profileId: string, except?: CasinoTableId) {
  if ([...this.pending.values()].some(p => p.profileId === profileId && p.tableId !== except && p.tableId !== 'roulette-1')) fail('already_seated', 'Wait for your current casino action to finish saving');
 ...
 async tick(_now?: number) {
  ...
  return this.enqueue(async () => {
   ...
   const r = this.roulette;
   if (!this.blocked(r.view.id)) {
    if (r.view.phase === 'betting' && this.now() >= r.view.deadline) { r.outcome = this.random(37); r.view.phase = 'spinning'; r.view.deadline = this.now() + ROULETTE_SPIN_MS; }
    if ((r.view.phase === 'spinning' && this.now() >= r.view.deadline) || r.view.phase === 'paused') {
     try {
      ...
      for (const [profileId, wallet] of wallets) this.hooks.wallet(profileId, wallet);
      r.view.result = r.outcome; r.view.history = [r.outcome!, ...r.view.history].slice(0, 12); r.view.phase = 'result'; r.view.deadline = this.now() + CASINO_RESULT_MS;
