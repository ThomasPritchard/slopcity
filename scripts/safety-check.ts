...
try {
 ...
 await page.goto(origin);await page.getByRole('textbox',{name:'What should we call you?'}).fill('n1gg3r');await page.getByRole('button',{name:'Choose your look',exact:true}).click();await page.getByText(PROFILE_NAME_ERROR,{exact:true}).waitFor();assert.equal(guestPosts,0,'blocked onboarding name is rejected before a request');
 await page.getByRole('textbox',{name:'What should we call you?'}).fill('Name check');await page.getByRole('button',{name:'Choose your look',exact:true}).click();await page.getByRole('button',{name:'Join the square',exact:true}).waitFor();
 await page.getByRole('textbox',{name:'Your name',exact:true}).fill('nigger9');await page.getByRole('button',{name:'Join the square',exact:true}).click();await page.getByText(PROFILE_NAME_ERROR,{exact:true}).waitFor();await page.screenshot({path:`${out}/blocked-character-name.png`});assert.deepEqual(errors,[]);
