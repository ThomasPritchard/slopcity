...
async function walk(axis:'x'|'z',target:number){for(let i=0;i<(chromiumRun?48:16);i++){const delta=target-(await position())[axis];if(Math.abs(delta)<.35)return;const key=axis==='x'?(delta>0?'d':'a'):(delta>0?'w':'s');await page.locator('#world').focus();await page.keyboard.down(key);await page.waitForTimeout(Math.min(1600,Math.max(70,Math.abs(delta)/4.2*1000)));await page.keyboard.up(key);await page.waitForTimeout(220);}throw Error(`Could not walk to ${axis}=${target}`);}
const visibleVariant=(p:Page,model:string)=>p.evaluate(model=>(window as any).shopScene.meshes.filter((m:any)=>m.name.includes(`/wear_${model}__`)&&m.isEnabled()).length,model);
try{
 ...
 if(chromiumRun){await page.bringToFront();}
 ...
 const arrival=await page.locator('.location').boundingBox();assert.ok(arrival);assert.ok(Math.abs(arrival.x+arrival.width/2-page.viewportSize()!.width/2)<2 && Math.abs(arrival.y+arrival.height/2-page.viewportSize()!.height/2)<2,'location announcement is centred');
 ...
 await other.waitForFunction(()=>(window as any).shopScene.meshes.some((m:any)=>m.name.includes('/wear_knit__')&&m.isEnabled()),undefined,{polling:100});
 ...
 await writeFile(`output/playwright/economy-${chromiumRun?'chromium':'webkit'}-results.json`,JSON.stringify({checkedAt:new Date().toISOString(),browser:chromiumRun?'headless Chromium, performance mode':'headless WebKit',checks:['1000 grant','real walk into shop','private 3D try-on','lost response recovers through server snapshot; replay charges once','buy separate from equip','remote equipment replication','three clothing slots','insufficient funds','cancel restores outfit','owned-only wardrobe','mobile layout','reload persistence',...(!chromiumRun?['centred arrival announcement fades out']:[]),'server presence accrual','salary paused outside town'],errors},null,2));
