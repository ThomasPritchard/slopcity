...
async function shot(name:string){await page.screenshot({path:`output/playwright/${name}.png`});}
...
try{
 ...
 await walk('x',-4.7);await walk('z',11.2);await walk('x',0);await walk('z',16.5);await walk('x',-8);await walk('z',17.25);
 console.log('Reached roulette through the town and entrance.');
 await page.getByRole('button',{name:'Open European roulette',exact:true}).click();
 ...
 await(await enabled('Place 10-credit bet'))[compact?'click':'tap']();await page.locator('.casino-your-bets li').waitFor();assert.equal((await wallet()).balance,990);assert.equal(await page.evaluate(()=>(window as any).casinoScene.getMeshByName('casino-chip-roulette-own-0')?.isEnabled()),true);
 ...
 await page.getByRole('button',{name:'Close casino table'}).click();await walk('z',16.25);await walk('x',3);
 ...
 await page.getByText('Round complete',{exact:true}).waitFor();const returned=Number((await page.locator('.blackjack-hand-outcome strong').innerText()).replace(/[^0-9]/g,''));assert.equal((await wallet()).balance,before.balance-10+returned);
 ...
 await walk('z',16.15);await walk('x',-2);await walk('z',22);
 await page.getByRole('button',{name:'Open Meridian reels · 3',exact:true}).click();const slotBefore=(await wallet()).balance;
