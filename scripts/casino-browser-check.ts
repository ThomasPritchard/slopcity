...
try{
 ...
 await page.getByRole('button',{name:'Numbers',exact:true}).click();await page.getByRole('button',{name:'0, zero',exact:true}).click();
 ...
 await(await enabled('Place 10-credit bet'))[compact?'click':'tap']();await page.getByRole('button',{name:'Your bets (1)',exact:true}).waitFor();await page.getByRole('button',{name:'Your bets (1)',exact:true}).click();await page.locator('.casino-bet-row').waitFor();assert.equal((await wallet()).balance,990);assert.equal(await page.evaluate(()=>(window as any).casinoScene.getMeshByName('casino-chip-roulette-1-own-0')?.isEnabled()),true);
 await page.locator('.casino-ready').click();
 ...
 await page.locator('.casino-round-status',{hasText:'Ball settling'}).waitFor();assert.equal(await page.locator('.casino-result-announcement').count(),0,'The result announcement waits for the physical landing');
 const rouletteResult=page.locator('.casino-round-status',{hasText:'Winning number:'});await rouletteResult.waitFor();const number=Number((await rouletteResult.innerText()).match(/Winning number: (\d+)/)![1]);await page.locator('.casino-result-announcement').waitFor();await shot('casino-personal-roulette-result');assert.equal((await wallet()).balance,number===0?1350:990);
 ...
 const before=await wallet();await enabled('Bet 10 credits');await mobileLayout('blackjack','Bet 10 credits');await(await enabled('Bet 10 credits')).tap();await page.locator('.casino-ready').waitFor();await page.locator('.casino-ready').click();assert.equal((await wallet()).balance,before.balance-10);
 await page.waitForFunction(()=>!!document.querySelector('.blackjack-hand .casino-card:not(.is-hidden)'),undefined,{timeout:30000});await page.waitForTimeout(450);
 ...
 const hit=page.getByRole('button',{name:'Hit',exact:true});if(await hit.isVisible()&&await hit.isEnabled()){
   const cardCount=await page.locator('.blackjack-hand .casino-card').count();await hit.click();await page.waitForFunction(count=>document.querySelectorAll('.blackjack-hand .casino-card').length>count,cardCount);
 ...
 await enabled('Spin for 10 credits');await mobileLayout('slots','Spin for 10 credits');await(await enabled('Spin for 10 credits')).tap();await page.getByRole('group',{name:'Reels spinning'}).waitFor();await page.getByRole('group',{name:'Reel result'}).waitFor();assert.equal(await page.getByRole('button',{name:'Spin for 10 credits',exact:true}).isEnabled(),true);
 ...
 const panel=await page.locator('.casino-panel').boundingBox();assert.ok(panel && panel.y>=230 && panel.y+panel.height<=845);
