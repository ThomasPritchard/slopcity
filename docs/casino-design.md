...
Updated 11 September 2026 for staged casino controls, player readiness and personal result feedback. Fictional credits, local browser build, existing guest identity and wallet. No new accounts, network exposure or payment services. `shared/casino.ts` owns the public transport types, limits, paytables and world anchors.
...
The panel uses fixed Play, How to play and Results views. Rules, number selection, accepted bets and other players’ hands use short pages where needed; actions remain in a persistent dock. The current instruction is visible in Play, and a newly arriving personal turn returns to that view. Town chat shares the dialog focus scope and stays accessible beside the game; `/` focuses its input without inserting the shortcut, Enter sends, and Escape exits chat without closing the table. Roulette also accepts typed stakes. Portrait retains a 28% world view above a 72% panel; short landscape keeps world and controls side by side.

A personal announcement appears only after a settled result is revealed. It reports net change (gross return minus all stakes), distinguishes loss/push/win, and labels poker awards as table chips. Repeated snapshots and reopening do not repeat the announcement; Results retains the most recent personal receipt in the current browser session, including settlement while the panel was closed. Reduced motion removes arrival animation. Roulette outside chips sit on their named felt areas, with separate positions for all six even-money bets, three dozens and three columns; number coverage remains highlighted privately.

## Rules and timing

House-game base stakes use 10-credit steps. Roulette allows up to 1,000 credits within the remaining round allowance and wallet balance; blackjack, slots and craps remain 10–100. Blackjack double/split adds the matching base stake. No wager cancellation or autoplay.

`table-presence { tableId, viewing }` records an explicit panel opening. A newly viewing nonparticipant gets up to five seconds to join, capped at five seconds after the first Ready; proximity alone does not delay play, closing removes the grace, and repeated opening cannot extend it. `round-ready { tableId, roundId }` marks a committed, connected participant ready. Once every participant is ready, a 1.5-second countdown can close betting before the normal deadline. The effective deadline is checked again at wager admission under the wallet lock. An additional roulette bet clears that player’s Ready; departed players no longer block early start. Reconnecting requires validated panel opening to reclaim roulette/craps readiness; old-session Ready does not carry over. Empty roulette windows remain available without spinning. House result holds are three seconds; physical spin/landing durations and blackjack decision time remain unchanged. Poker uses the same intent controls between hands and still requires at least two funded players.

- **European roulette:** up to 20 seconds betting (Ready can start early), five seconds spinning, settlement followed by a 500 ms landing lead and 4.2-second simulated landing, then three seconds result. Single zero; straight, adjacent split (including 0/1, 0/2, 0/3), street, zero trios {0,1,2}/{0,2,3}, corner, first four {0,1,2,3}, six-line, colour, parity, low/high, dozen and column. `numbers` must equal a canonical selection for its declared kind, with no duplicates. Zero loses every outside bet. Profit multipliers are in the shared contract; gross return adds the stake. Limit each player to 20 accepted bets and 1,000 total credits per table round. Start the next betting window after the result interval.
- **Blackjack:** up to 20 seconds betting (Ready can start early), 30 seconds per hand action, three seconds result. Fresh shuffled six-deck shoe each round; server crypto Fisher–Yates. Dealer peeks for a natural, stands on soft 17; natural pays 3:2 profit, ordinary win 1:1, push returns stake. One split, for identical ranks or any two ten-value cards (10, J, Q, K), at most two hands. Split aces receive one card each and stand; split 21 is ordinary 21. Double allowed on the first two cards, including non-ace split hands, draws once and stands. No insurance or surrender. Natural/bust hands finish automatically. Timeout, disconnect or walking away stands active hands; accepted bets finish normally. A reconnected guest can reclaim its still-active seat. Only one initial bet per seat per round. No automatic stake for the next round.
- **Slots:** each of three independent reels has 16 equally likely stops: seven cherry, four lemon, three bar, two seven. Three matches pay gross 3×/8×/20×/50× respectively; exactly two cherries anywhere returns 1×; otherwise zero. Resolve and persist before a 2.5-second visual spin, then reveal. The connected owner can explicitly spin again immediately; otherwise the machine releases after six seconds. No pending-credit animation is presented as a committed win. One occupant and one spin per machine; a profile may occupy only one blackjack/slot station at once.
