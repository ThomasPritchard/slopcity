...
import {
  ...
  POKER_MIN_BUY_IN, POKER_SMALL_BLIND,
  type PokerMove, type PokerSeatView, type PokerView,
} from '../../shared/poker';
import { PagedItems, StageTabs } from './CasinoViews';
...
export function PokerGame({ table, now, profileId, balance, privateState, busy, send, actionHost }: PokerGameProps) {
  ...
  const reasonId = useId(), raiseId = useId(), raiseHelpId = useId();
  const [stage,setStage] = useState<'hand'|'table'>(privateState.poker ? 'hand' : 'table');
  const [raising,setRaising] = useState(false);
  const raiseInput = useRef<HTMLInputElement>(null);
  ...
  const joinOpen = betweenHands && (!deadline || now < deadline);
  const canJoin = !hasPlace && joinOpen && selectedSeat !== undefined && balance >= buyIn && !busy;
  ...
  useEffect(() => { setRaising(false); if (actions) setStage('hand'); }, [actions?.turnId]);
  useEffect(() => { if (raising) raiseInput.current?.focus({preventScroll:true}); }, [raising]);
  ...
  const controls = player?.canRejoin ? <button type="button" className="casino-primary poker-wide-action" disabled={!canRejoin} aria-describedby={reasonId} onClick={() => { if (canRejoin) send({ action: 'poker-rejoin', tableId: table.id, escrowId: player.escrowId }); }}>{busy ? 'Rejoining…' : 'Rejoin your seat'}</button>
    : !hasPlace ? <>
      ...
      <div className="poker-basic-actions" hidden={raising} role="group" aria-label="Your poker actions">
        ...
        {actions.canRaise && <button type="button" className="casino-secondary" disabled={!canAct} onClick={()=>setRaising(true)}>Raise…</button>}
      </div>
      {actions.canRaise && raising && <div className="poker-raise-controls">
        <label className="poker-raise-field" htmlFor={raiseId}><span>Raise total</span><input ref={raiseInput} id={raiseId} type="number" inputMode="numeric" min={actions.minRaiseTo} max={actions.maxRaiseTo} step={1} value={raiseValue} disabled={!canAct} aria-describedby={raiseHelpId} aria-invalid={!validRaise} onChange={event => setRaiseChoice({ turnId: actions.turnId, value: event.target.value })} /></label>
        ...
        <button type="button" className="casino-secondary poker-raise-back" onClick={()=>setRaising(false)}>Back</button>
      </div>}
  ...
  return <div className={`poker-game${actionHost ? ' has-action-dock' : ''}`} data-phase={table.phase}>
    ...
    <div className="casino-round-status"><span><i aria-hidden="true" />{phaseLabel}</span>{deadline > 0 && table.phase !== 'paused' && <span className="casino-countdown" aria-label={remaining ? `${remaining} seconds remaining` : 'Waiting for the table'}>{remaining ? `${remaining}s` : 'Waiting…'}</span>}</div>

    <StageTabs<typeof stage> value={stage} onChange={setStage} label="Poker views" options={[{value:'hand',label:'Community cards'},{value:'table',label:`Players (${table.seats.length}/6)`}]}/>
    <div className="casino-stage">
    <p id={reasonId} className={stage==='table'?'sr-only':'casino-instruction poker-action-reason'}>{raising && actions ? `Raise total includes ${chips(ownSeat?.bet ?? 0)} already bet. Enter ${chips(actions.minRaiseTo)}–${chips(actions.maxRaiseTo)}.` : yourTurn ? actions?.canCall ? `Call adds ${chips(actions.callAmount)} chips. Fold leaves this hand; Raise opens an amount step.` : 'Check to stay in without adding chips, or choose a raise.' : actionReason}</p>
    {stage==='hand' ? <section className="poker-board" aria-label="Community cards and pot"><div className="casino-section-heading"><h3>Community cards</h3><span className="poker-pot">Pot <strong>{chips(table.pot)}</strong> chips</span></div><div className="casino-cards poker-board-cards">{Array.from({length:5},(_,index)=><PokerCard key={index} card={table.board[index]??null} empty={!table.board[index]}/>)}</div></section>
    : <section className="poker-seating" aria-label="Players and seats"><PagedItems items={Array.from({length:6},(_,seat)=>seat)} label="Seat" render={seatNumber=>{
      const seat=table.seats.find(entry=>entry.seat===seatNumber); const active=table.activeSeat===seatNumber && bettingPhases.includes(table.phase);
      return seat ? <div key={seatNumber} className={`poker-seat${seat.player.profileId===profileId?' is-yours':''}${active?' is-active':''}`}><div className="poker-seat-info"><div className="poker-seat-heading"><span>Seat {seatNumber+1}</span><span className="poker-seat-markers">{table.button===seatNumber&&<abbr title="Dealer button">D</abbr>}{table.smallBlindSeat===seatNumber&&<abbr title="Small blind">SB</abbr>}{table.bigBlindSeat===seatNumber&&<abbr title="Big blind">BB</abbr>}</span></div><strong className="poker-player-name">{seat.player.name}{seat.player.profileId===profileId?' · you':''}</strong><span className="poker-seat-stack">{chips(seat.stack)} chips · {seatStatus(seat,active)}</span><span className="poker-seat-bet">Bet {chips(seat.bet)} · in pot {chips(seat.committed)}</span></div>{seat.cards.length>0&&<div className="poker-seat-cards" aria-label={`${seat.player.name}’s public cards`}>{seat.cards.map((card,index)=><PokerCard key={index} card={card}/>)}</div>}</div>
      : <button key={seatNumber} type="button" className="poker-seat is-empty" aria-label={`Select seat ${seatNumber+1}`} aria-pressed={!hasPlace&&selectedSeat===seatNumber} disabled={busy||hasPlace||!betweenHands} onClick={()=>setSeatChoice(seatNumber)}><strong>Seat {seatNumber+1} · Open seat</strong><small>{!hasPlace&&selectedSeat===seatNumber?'Selected · set your buy-in below':'Choose this seat'}</small></button>;
    }}/></section>}
    </div>
    ...
    {actions?.canRaise && <p id={raiseHelpId} className="sr-only">Raise total includes {chips(ownSeat?.bet ?? 0)} already bet. Enter {chips(actions.minRaiseTo)}–{chips(actions.maxRaiseTo)}.</p>}
  </div>;
}
