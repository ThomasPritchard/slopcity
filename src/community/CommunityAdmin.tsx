...
type AdminData = { images: CommunitySubmission[]; programme: Programme; settings?: ProgrammeSettings; configured: boolean };
type AdminTab = 'queue' | 'programme' | 'upload' | 'preview' | 'safety';

function ProgrammeEditor({ programme, savedSettings, onSaved }: { programme: Programme; savedSettings?: ProgrammeSettings; onSaved(): void }) {
 const [settings, setSettings] = useState<ProgrammeSettings>(() => { const saved = savedSettings ?? programme; return { mode: saved.platform === 'twitch' ? 'intermission' : saved.mode, platform: saved.platform, twitchChannel: saved.twitchChannel, youtubeVideoId: saved.youtubeVideoId, schedule: saved.schedule }; });
 ...
 const detection = programme.liveDetection;
 const detectionText = !detection || detection.status === 'unconfigured' ? 'Automatic Twitch checks need setup. Add the Twitch app credentials to the server.' : detection.status === 'live' ? 'BridgeMind is live on Twitch. The town switches to his stream automatically.' : detection.status === 'offline' ? 'BridgeMind is offline on Twitch. The town is showing the programme below.' : detection.status === 'checking' ? 'Checking whether BridgeMind is live on Twitch…' : 'Twitch could not be checked. Keeping the last confirmed programme and retrying automatically.';
 return <form className="community-form community-programme-form" onSubmit={event => void save(event)} aria-label="Edit cinema programme"><div className="community-section-heading"><span className="community-kicker">THE SHARED SCREEN</span><h3>Tonight’s programme</h3><p>Twitch takes over automatically when BridgeMind goes live. Between streams, show the community reel or a YouTube video you choose.</p><p role="status" data-testid="twitch-detection">{detectionText}{detection?.checkedAt && <> Last checked {new Date(detection.checkedAt).toLocaleTimeString()}.</>}</p></div><fieldset disabled={busy}>
  <div className="community-form-row"><label>Live platform<select aria-label="Live platform" value={settings.platform} onChange={event => { setSettings(current => ({ ...current, platform: event.target.value as Programme['platform'], mode: 'intermission' })); setNotice(''); }}><option value="twitch">Automatic Twitch + community reel</option><option value="youtube">YouTube between Twitch streams</option></select></label><label>Between streams<select aria-label="Screen mode" value={settings.mode} disabled={settings.platform === 'twitch'} onChange={event => field('mode', event.target.value as Programme['mode'])}><option value="intermission">Community reel</option><option value="live">Chosen YouTube video</option></select></label></div>
